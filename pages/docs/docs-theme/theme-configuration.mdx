...
主题通过 [theme.config.jsx](/docs/docs-theme/start#create-docs-theme-config) 文件配置.
它应该导出一个包含你的配置的对象，例如:
...
你可以参考 [`useConfig`](/docs/docs-theme/api/use-config) API 部分以获取更多关于 `useConfig` 钩子和 `frontMatter` 对象的信息.
...
结合 [分隔符](/docs/docs-theme/page-configuration#separators) 项，你可以通过使用`sidebar.titleComponent`选项自定义侧边栏内容的渲染方式：
